...
def _load_main_supervisor_actions():
    ...
    ), "Main supervisor automation v7_5_main_supervisor not found"
...
def _extract_cooling_variable_templates(actions):
    ...
    assert (
        not missing
    ), f"Missing expected Section 2 cooling variable templates: {sorted(missing)}"
    return templates
...
def test_section2_cooling_setpoint_and_threshold_doctrine():
    ...
    assert (
        templates["m_off_at"] == "{{ 74 if away else (62 if is_master_sleep else 68) }}"
    )
    assert (
        templates["m_on_at"] == "{{ 76 if away else (66 if is_master_sleep else 72) }}"
    )
