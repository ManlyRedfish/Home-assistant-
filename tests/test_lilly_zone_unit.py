"""Focused contract tests for the unwired Lilly supervisor extraction."""
...
LILLY_UNIT = ROOT / "supervisor_zones" / "lilly.yaml"
...
def test_lilly_unit_is_an_exact_unwired_extraction():
    supervisor = load_supervisor(AUTOMATIONS)
    source_groups = zone_action_groups(supervisor)["climate.lilly_air"]
...
def test_lilly_unit_is_single_zone_and_preserves_off_actions():
    ...
    assert climate_targets == {"climate.lilly_air"}
    assert sum(
