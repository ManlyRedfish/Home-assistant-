    ...
    data.update(
        {
            "Timestamp": timestamp,
            "LR_Temp_Truth": "66",
            "Master_Temp_Truth": "64",
            "Lincoln_Temp_Truth": "69",
            "Lilly_Temp_Truth": "70",
            "LR_Air_Setpoint": "61",
            "Master_Air_Setpoint": "61",
            "Lincoln_Air_Setpoint": "61",
            "Lilly_Air_Setpoint": "61",
            "LR_Air_Mode": "cool",
            "Master_Air_Mode": "cool",
            "Lincoln_Air_Mode": "cool",
            "Lilly_Air_Mode": "cool",
        }
    )
    data.update(extra)
...
def test_annotation_interval_marks_historical_rows(tmp_path: Path) -> None:
    ...
    write_csv(
        annotations,
        ["start_local", "end_local", "kind", "note", "created_at"],
        [
            {
...
def test_unusual_temperature_or_setpoint_is_observation_not_contamination(
    tmp_path: Path,
) -> None:
    path = tmp_path / "telemetry.csv"
    header = BASE_HEADER + ["Supervisor_Enabled", "Manual_Override_State"]
...
def test_annotation_schema_validation(tmp_path: Path) -> None:
    ...
    write_csv(
        path, ["start_local", "kind"], [{"start_local": "2026-06-05", "kind": "other"}]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "2026-06-05T12:34:56+00:00",
            datetime(2026, 6, 5, 12, 34, 56, tzinfo=timezone.utc),
        ),
        (
            "2026-06-05T12:34:56-04:00",
            datetime(2026, 6, 5, 12, 34, 56, tzinfo=timezone(timedelta(hours=-4))),
        ),
    ],
)
